...
class RSequence(pt.composites.Selector):
    ...
    """
    Reactive sequence overidding fallback, reversing its output.
    """
    ...
    def tick(self):
        ...
        for child in self.children:
            for node in child.tick():
                yield node
                if node is child:
                    if node.status == pt.common.Status.RUNNING or node.status == pt.common.Status.FAILURE: # NOTE: changed here
        ...
        self.status = pt.common.Status.SUCCESS # NOTE: changed here
        try:
