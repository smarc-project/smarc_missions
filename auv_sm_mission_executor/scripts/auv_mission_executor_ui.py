...
import auv_mission_executor.page_utils
...
if __name__ == '__main__':
    ...
    strands_webserver.client_utils.set_http_root(roslib.packages.get_pkg_dir('state_machine') + '/www', prefix=prefix)
    ...
    script = auv_mission_executor.page_utils.get_schedule_display("#top_right_div")
    script += auv_mission_executor.page_utils.get_service_button("Cancel active task", "sm_reset", "#button_left_div")
    script += auv_mission_executor.page_utils.get_service_button("Cancel all tasks", "cancel_all_tasks", "#button_right_div")

    auv_mission_executor.page_utils.generate_interface_page(page, right=right_div, script=script)
